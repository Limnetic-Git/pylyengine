...
class Slotlist:
    def __init__(self, ui_layer, array, rel_x, rel_y, slot_size, distance_between=5):
        ...
        self.item_buffer = {'item': None, 'count': 0, 'parent_slot': [0, 0]}
    ...
    def transfer(self, target_slot_coor):
        ...
        if not target_slot['item']:
            self.array[target_slot_coor[0]][target_slot_coor[1]] = self.item_buffer
            self.delete_items_from_slot(self.item_buffer['parent_slot'], self.item_buffer['count'])
            self.item_buffer = {'item': None, 'count': 0, 'parent_slot': [0, 0]}

        elif self.item_buffer['parent_slot'] == target_slot_coor:
            self.item_buffer = {'item': None, 'count': 0, 'parent_slot': [0, 0]}

        elif target_slot['item'] == self.item_buffer['item']:
            if target_slot['count'] != target_slot['item'].stack:
                if target_slot['item'].stack > target_slot['count'] + self.item_buffer['count']:
                    self.delete_items_from_slot(self.item_buffer['parent_slot'], self.item_buffer['count'])
                    target_slot['count'] += self.item_buffer['count']
                    self.item_buffer = {'item': None, 'count': 0, 'parent_slot': [0, 0]}
                else:

                    self.delete_items_from_slot(self.item_buffer['parent_slot'], self.item_buffer['count'])
                    self.item_buffer['count'] -= target_slot['item'].stack - target_slot['count']
                    target_slot['count'] = target_slot['item'].stack
    ...
    def update(self):
        ...
        for x in range(len(self.array)):
            for y in range(len(self.array[x])):
                ...
                if window.mouse_x >= slot_x and window.mouse_x <= slot_x + self.width and window.mouse_y >= slot_y and window.mouse_y <= slot_y + self.height:
                    render_mode = "hover"
                    if raylib.IsMouseButtonReleased(0):
                        self.selected_slot = [x, y]
                        if not self.item_buffer['item']:
                            self.item_buffer = {'item': self.array[x][y]['item'],
                                                'count': 1,
                                                'parent_slot': [x, y],
                                            }
                        else:
                    ...
                    elif raylib.IsMouseButtonReleased(1):
                        self.selected_slot = [x, y]
                        self.item_buffer = {'item': self.array[x][y]['item'],
                    ...
                    elif raylib.IsMouseButtonReleased(2):
                        self.selected_slot = [x, y]
                        if self.array[x][y]['count'] != 1:
                            self.item_buffer = {'item': self.array[x][y]['item'],
                                                'count': self.array[x][y]['count'] // 2,
                                                'parent_slot': [x, y],
                                            }
                ...
                if self.array[x][y]['item']:
                    ...
                    item_counter = self.array[x][y]['count']
                    if self.item_buffer['parent_slot'] == [x, y]:
                        item_counter -= self.item_buffer['count']

                    if item_counter > 0:
                        texture_element = Texture(self.ui_layer,
                                                  texture_element_x,
                                                  texture_element_y,
                                                  texture_element_width,
                                                  texture_element_height,
                                                  self.array[x][y]['item'].texture_name,
                                                  raylib.WHITE,
                                                 )
                        text_element = Text(self.ui_layer,
